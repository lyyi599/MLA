...
class ViT_Tuner(nn.Module):
    ...
    def __init__(self, cfg, vit_model, num_classes):
        ...
        use_global_adapt = cfg.global_adapt
        use_lora = cfg.lora
        ...
        if (use_adapter or use_adaptformer or use_lora or use_lora_mlp or use_global_adapt) and (adapter_dim is None):
        ...
        if use_global_adapt:
            global_parallel_list = nn.ModuleList([
                *[None] * (_start),
                *[AdaptFormer(in_dim=emb_dim, bottle_dim=adapter_dim, dtype=dtype) for _ in range(_start, _end)],
                *[None] * (n_layers - _end)
            ])
            global_serial_list = nn.ModuleList([
                *[None] * (_start),
                *[Global_Serial(in_dim=emb_dim, bottle_dim=adapter_dim, dtype=dtype) for _ in range(_start, _end)],
                *[None] * (n_layers - _end)
            ])
        else:
            global_parallel_list = nn.ModuleList([None] * n_layers)
            global_serial_list = nn.ModuleList([None] * n_layers)
        ...
        self.global_serial_list = global_serial_list
        self.global_parallel_list = global_parallel_list
    ...
    def forward(self, x, tuner=None, head=None):
        ...
        for i in range(n_layers):
            ...
            if tuner is not None:
                ...
                global_serial = tuner.global_serial_list[i]
                global_parallel = tuner.global_parallel_list[i]
                ...
                vpt = adapter = adaptformer = global_serial = global_parallel = lora = lora_mlp = ssf_attn = ssf_mlp = ssf_ln = masked_linear = None
            ...
            if global_parallel is not None:
                x = x + global_parallel(identity)
