...
class ModifiedLogitAdjustedLoss(nn.Module):
    def __init__(self, cls_num_list, tau=1.0, epsilon=1e-8):
        super().__init__()
        cls_num_ratio = cls_num_list / torch.sum(cls_num_list)
        log_cls_num = torch.log(cls_num_ratio)
        self.log_cls_num = log_cls_num
    ...
    def forward(self, logit, target):
        ...
        logit_adjusted = torch.zeros_like(logit)
        for i in range(batch_size):
            # plus max
            logit_ratio = torch.max(torch.tensor(1.0), logit[i] / (logit[i, target[i]] + self.epsilon))
            # plus min
            # logit_ratio = torch.min(torch.tensor(1.0), logit[i] / (logit[i, target[i]] + self.epsilon))
            # multi (1-logits)
            # logit_ratio = (torch.tensor(1.0) - logit[i] / (logit[i, target[i]] + self.epsilon))
            # plus square
            # logit_ratio = logit[i] / (logit[i, target[i]] + self.epsilon)
            # logit_ratio = logit[i, target[i]] / (logit[i] + self.epsilon)
            logit_adjusted[i] = logit[i] + self.tau * self.log_cls_num + logit_ratio
        return F.cross_entropy(logit_adjusted, target)


class ModifiedLogitAdjustedLoss1(nn.Module):
    def __init__(self, cls_num_list, tau=1.0, epsilon=1e-8, power=1.0):
        super().__init__()
        cls_num_ratio = cls_num_list / torch.sum(cls_num_list)
        log_cls_num = torch.log(cls_num_ratio)
        self.log_cls_num = log_cls_num
    ...
    def forward(self, logit, target):
        ...
        for i in range(batch_size):
            ...
            logit_ratio = torch.min(torch.tensor(5.0), torch.max(torch.tensor(1.0), logit[i] / (logit[i, target[i]] + self.epsilon)))
            logit_adjusted[i] = logit[i] + self.tau * self.log_cls_num + logit_ratio ** self.power
        return F.cross_entropy(logit_adjusted, target)


class ModifiedLogitAdjustedLoss2(nn.Module):
    def __init__(self, cls_num_list, tau=1.0, epsilon=1e-8):
        super().__init__()
        cls_num_ratio = cls_num_list / torch.sum(cls_num_list)
        log_cls_num = torch.log(cls_num_ratio)
    ...
    def forward(self, logit, target):
        batch_size = logit.shape[0]
        logit_adjusted = torch.zeros_like(logit)
        logit_ratio_prob = F.softmax(logit, dim = 1)
        for i in range(batch_size):
            # plus max
            # logit_ratio = torch.max(torch.tensor(1.0), logit_ratio_prob[i] / (logit_ratio_prob[i, target[i]] + self.epsilon))
            
            # logit_ratio = logit[i] / (logit[i, target[i]] + self.epsilon)
            logit_ratio = torch.tensor(1.0)
            # 限制 logit_ratio 的范围在 1 到 10 之间
            logit_ratio = torch.clamp(logit_ratio, min=1.0, max=100.0)
