...
class Evaluator:
    ...
    def __init__(self, cfg, many_idxs=None, med_idxs=None, few_idxs=None, sorted_indices=None):
        self.cfg = cfg
    ...
    def save_pickle_epoch(self, mo, gt, idx, path):
        """
        ...
        """
        pred = mo.max(1)[1]  # 获取预测的标签
        conf = torch.softmax(mo, dim=1)  # 获取每个类别的置信度，shape: [batch_size, num_classes]
        labels = gt.data.cpu().numpy().tolist()  # 将真实标签转为列表格式

        # 将结果打包为字典，键为 "pred", "conf", "label", "idx", "path"
        new_results = {
            "pred": pred.data.cpu().numpy().tolist(),  # 预测的类别
            "conf": conf.data.cpu().numpy().tolist(),  # 每个样本对于所有类别的置信度
            "label": labels,  # 真实标签
            "idx": idx,  # 样本索引
            "path": path  # 样本路径
        }
    ...
    def save_pickle_final(self, file_path):
        # 将结果保存回pickle文件
        with open(os.path.join(file_path, "results.pickle"), 'wb') as f:
    ...
    def evaluate(self):
        ...
        for i in self.sorted_indices:
            self.cls_accs_sort.append(cls_accs[i])
